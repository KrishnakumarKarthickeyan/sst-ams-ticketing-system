...
export default function AdminManagersPage() {
  ...
  return (
    <div className="space-y-6 text-xs text-ink">
      ...
      {/* Command ribbon + KPI strip (ui-ux-pro-max) — oversight at a glance */}
      {(() => {
        const active = managers.filter(m => m.active).length;
        const overseen = managers.reduce((s, m) => s + (m.ticketCount || 0), 0);
        const critical = managers.reduce((s, m) => s + (m.criticalCount || 0), 0);
