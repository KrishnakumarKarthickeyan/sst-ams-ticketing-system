...
export default function CustomerTicketsPage() {
  ...
  const getSlaBadge = (t: any) => {
    ...
    return (
      <Badge variant="outline" className="bg-surface-subtle text-ink-muted border-line text-[11px] font-bold rounded px-1.5 py-0">
        N/A
      </Badge>
    );
  };
  ...
  const getPriorityBadge = (prio: string) => {
    if (prio === 'Critical') {
      return (
        <Badge className="bg-red-600 hover:bg-red-600 text-white text-[11px] font-bold rounded px-1.5 py-0">
          Critical
        </Badge>
      );
    }
    if (prio === 'High') {
      return (
        <Badge variant="outline" className="bg-orange-50 text-orange-700 border-orange-200 text-[11px] font-bold rounded px-1.5 py-0">
          High
        </Badge>
      );
    }
    ...
    return (
      <Badge variant="outline" className="bg-surface-muted text-ink-secondary border-line text-[11px] font-bold rounded px-1.5 py-0">
        Low
      </Badge>
    );
  };
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <Badge variant="outline" className={`text-[11px] font-bold rounded px-1.5 py-0 ${classes[status] || 'bg-surface-muted text-ink-secondary'}`}>
    ...
    <TooltipProvider>
      <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between border-b border-line pb-5">
          <div>
            <h1 className="text-xl font-bold tracking-tight text-ink uppercase flex items-center gap-1.5">
              <FolderOpen size={18} />
              SAP Service Desk Registry
            </h1>
            <p className="text-xs text-ink-secondary font-medium mt-0.5">
              Review and manage support request records assigned under organization <span className="font-bold text-ink">{customerCompany}</span>.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Link
              href="/customer/create-ticket"
              className="px-3.5 py-2 bg-ink hover:bg-ink text-white rounded-lg font-bold uppercase tracking-wider text-[11px] flex items-center gap-1.5 transition-all shadow-card"
            >
              <Plus size={13} />
              Submit Ticket
            </Link>
            <Button
              ...
              variant="outline"
              className="h-8.5 text-[11px] font-bold uppercase tracking-wider border-line hover:border-line-strong flex items-center gap-1.5"
            >
        ...
        <div className="space-y-4">
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-surface border border-line rounded-lg p-3 shadow-card">
            {/* Search */}
            <div className="relative w-full md:max-w-md">
              ...
              <input
                ...
                className="w-full bg-surface-muted border border-line rounded-lg pl-9 pr-4 py-1.5 text-xs text-ink focus:outline-none focus:border-brand"
              />
            </div>
            ...
            <div className="flex items-center gap-2">
              <ToggleGroup type="single" value={viewMode} onValueChange={(val) => { if (val) handleViewModeChange(val as 'table' | 'card'); }}>
                <ToggleGroupItem value="card" aria-label="Card View" className="h-8 px-2 border border-line bg-surface text-ink-secondary data-[state=on]:bg-surface-subtle data-[state=on]:text-ink">
                  <LayoutGrid size={14} />
                </ToggleGroupItem>
                <ToggleGroupItem value="table" aria-label="Table View" className="h-8 px-2 border border-line bg-surface text-ink-secondary data-[state=on]:bg-surface-subtle data-[state=on]:text-ink">
                  <List size={14} />
                </ToggleGroupItem>
              </ToggleGroup>
            </div>
        ...
        {loading ? (
          <div className={viewMode === 'card' ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" : "w-full space-y-2"}>
            {Array.from({ length: 6 }).map((_, idx) => (
              viewMode === 'card' ? (
                <Card key={idx} className="border-line shadow-card bg-surface overflow-hidden p-5 space-y-4">
                ...
                <div key={idx} className="flex gap-4 p-3 border-b border-line items-center">
                  <Skeleton className="h-4 w-16" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {currentTickets.length === 0 ? (
              <div className="col-span-full text-center py-12 bg-surface border border-line rounded-lg space-y-3 shadow-card">
                ...
                <p className="text-sm font-medium text-ink-secondary">No tickets found matching your selection.</p>
              </div>
            ) : (
                    ...
                    <div className="p-5 space-y-4">
                      ...
                      <div className="space-y-1">
                        <Link
                          ...
                          className="text-sm font-bold text-ink hover:text-ink hover:underline transition-all line-clamp-1 block leading-snug"
                        >
                          {t.title}
                        </Link>
                      ...
                      {hasSla && (
                        <div className="text-[11px] text-ink-secondary grid grid-cols-2 gap-y-1 gap-x-2 bg-surface-muted p-2 rounded border border-line mt-2">
                          <div>Due: <strong className="text-ink">{new Date(t.slaDueAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</strong></div>
                          <div>Remaining: <strong className={t.status === 'Resolved' || t.status === 'Closed' ? 'text-success' : 'text-ink'}>{remainingTimeStr}</strong></div>
                          {escalationLevel > 0 && (
                            <div className="col-span-2 text-critical font-bold flex items-center gap-0.5">
                      ...
                      <div className="pt-3 border-t border-line flex items-center justify-between text-[11px]">
                        ...
                        <div className="flex items-center gap-1 text-ink-secondary">
                          ...
                          <span>Logged: <strong className="text-ink">{actualHours.toFixed(1)}h</strong></span>
                        </div>
                      </div>
                    </div>

                    <div className="px-5 py-3 bg-surface-muted/60 border-t border-line flex items-center justify-between text-[11px] text-ink-secondary">
                      <span className="flex items-center gap-1">
                        <Calendar size={11} />
                      ...
                      <Link
                        href={`/customer/tickets/${t.id}`}
                        className="text-[11px] font-bold uppercase tracking-wider text-ink-secondary hover:text-ink transition flex items-center gap-1"
                      >
                        Manage Ticket
        ...
        {viewMode === 'card' && totalPages > 1 && (
          <div className="flex items-center justify-between border border-line rounded-lg p-4 bg-surface shadow-card text-[11px]">
            <div className="text-ink-secondary font-bold">
              ...
              <span className="text-ink">{Math.min(indexOfLastItem, totalItems)}</span> of{' '}
              <span className="text-ink font-black">{totalItems}</span> matching records
            </div>
            <div className="flex gap-1">
              <Button
                ...
                className="h-8 px-2 flex items-center border-line"
              >
                <ChevronLeft size={14} />
                <span>Prev</span>
              </Button>
              <div className="flex items-center px-3 border border-line rounded-lg bg-surface-muted font-bold">
        ...
        <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
          <DialogContent className="sm:max-w-md bg-surface border border-line text-xs">
            <DialogHeader>
            ...
            <div className="space-y-4 py-3">
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-ink-secondary uppercase tracking-wider block">Ticket Subject</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full bg-surface border border-line rounded-lg p-2 text-xs text-ink focus:outline-none focus:border-brand"
                />
              </div>
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-ink-secondary uppercase tracking-wider block">Detailed Description</label>
                <textarea
                  rows={4}
                  required
                  ...
                  className="w-full bg-surface border border-line rounded-lg p-2.5 text-xs text-ink focus:outline-none focus:border-brand"
                />
              </div>
            </div>
            <DialogFooter className="gap-2">
              <Button variant="outline" className="h-8 text-[11px] uppercase font-bold" onClick={() => setIsEditOpen(false)}>
                Cancel
              </Button>
              <Button variant="default" className="h-8 text-[11px] uppercase font-bold bg-ink text-white" onClick={submitEdit}>
                Save Changes
              </Button>
            </DialogFooter>
          </DialogContent>
        ...
        <Dialog open={isCommentOpen} onOpenChange={setIsCommentOpen}>
          <DialogContent className="sm:max-w-md bg-surface border border-line text-xs">
            ...
            <div className="space-y-4 py-3">
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-ink-secondary uppercase tracking-wider block">Comment Content</label>
                <textarea
                  rows={4}
                  required
            ...
            <DialogFooter className="gap-2">
              <Button variant="outline" className="h-8 text-[11px] uppercase font-bold" onClick={() => setIsCommentOpen(false)}>
                Cancel
              </Button>
              <Button variant="default" className="h-8 text-[11px] uppercase font-bold bg-ink text-white" onClick={submitComment}>
                Submit Comment
              </Button>
            </DialogFooter>
          </DialogContent>
        ...
        <Dialog open={isAttachmentOpen} onOpenChange={setIsAttachmentOpen}>
          <DialogContent className="sm:max-w-md bg-surface border border-line text-xs">
            ...
            <div className="space-y-4 py-3">
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-ink-secondary uppercase tracking-wider block">File Name</label>
                <input
                  type="text"
                  required
              ...
              {isUploading && (
                <div className="space-y-1">
                  <div className="flex justify-between font-bold text-[11px] text-ink-secondary">
                    <span>Uploading attachment...</span>
                    <span>100%</span>
                  </div>
                  <div className="w-full bg-surface-subtle rounded-full h-1.5 overflow-hidden">
                    <div className="bg-ink h-1.5 rounded-full animate-pulse w-full"></div>
                  </div>
                </div>
              )}
            </div>
            <DialogFooter className="gap-2">
              <Button variant="outline" className="h-8 text-[11px] uppercase font-bold" onClick={() => setIsAttachmentOpen(false)} disabled={isUploading}>
                Cancel
              </Button>
              <Button variant="default" className="h-8 text-[11px] uppercase font-bold bg-ink text-white" onClick={submitAttachment} disabled={isUploading || !attachFileName.trim()}>
                Upload File
              </Button>
            </DialogFooter>
          </DialogContent>
        ...
        <Dialog open={isEscalateOpen} onOpenChange={setIsEscalateOpen}>
          <DialogContent className="sm:max-w-md bg-surface border border-line text-xs">
            ...
            <div className="space-y-4 py-3">
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-ink-secondary uppercase tracking-wider block">Severity Level</label>
                <select
              ...
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-ink-secondary uppercase tracking-wider block">Escalation Rationale</label>
                <textarea
                  rows={3}
                  required
              ...
              <div className="space-y-2">
                <label className="text-[11px] font-bold text-ink-secondary uppercase tracking-wider block">Escalation Attachments</label>
                <div className="relative border-2 border-dashed border-line rounded-lg p-4 bg-surface-muted/60 hover:bg-orange-50/30 hover:border-orange-300 transition flex flex-col items-center justify-center gap-1 cursor-pointer group">
                  <input
                    type="file"
                    multiple
                  ...
                  />
                  <Upload size={16} className="text-ink-muted group-hover:text-warning transition" />
                  <span className="text-xs text-ink-secondary font-medium">Select files to attach</span>
                ...
                {escalateFiles.length > 0 && (
                  <div className="grid grid-cols-1 gap-2 mt-2">
                    {escalateFiles.map((pf) => (
                      <div key={pf.id} className="relative flex items-center gap-3 p-2 bg-surface-muted border border-line rounded-lg">
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-medium text-ink truncate pr-6">{pf.file.name}</p>
            ...
            <DialogFooter className="gap-2">
              <Button variant="outline" className="h-8 text-[11px] uppercase font-bold" onClick={() => setIsEscalateOpen(false)}>
                Cancel
              </Button>
              <Button variant="default" className="h-8 text-[11px] uppercase font-bold bg-ink text-white" onClick={submitEscalate}>
                Submit Escalation
              </Button>
            </DialogFooter>
          </DialogContent>
        ...
        <Dialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
          <DialogContent className="sm:max-w-md bg-surface border border-line text-xs">
            ...
            <div className="space-y-4 py-3">
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-ink-secondary uppercase tracking-wider block">Deletion Reason</label>
                <textarea
                  rows={3}
                  required
            ...
            <DialogFooter className="gap-2">
              <Button variant="outline" className="h-8 text-[11px] uppercase font-bold" onClick={() => setIsDeleteOpen(false)}>
                Cancel
              </Button>
              <Button variant="destructive" className="h-8 text-[11px] uppercase font-bold bg-red-650 hover:bg-red-750 text-white" onClick={submitDelete}>
                Request Deletion
              </Button>
            </DialogFooter>
          </DialogContent>
