...
async function test() {
  ...
  console.log("1. Setting up assignments, closure request, actual hours...");
  ...
  const { data: insertedReq } = await supabase.from('ticket_closure_requests').insert({
    ticket_id: ticketId,
  ...
  console.log("3. Attempting to run mapDbTicket on ticket with assignments...");
  try {
    const mapped = mapDbTicket(dbTickets[0], dbProfiles, dbContacts || []);
    console.log("✅ Success! Mapped Ticket consultantEfforts length:", mapped.consultantEfforts.length);
    console.log("Mapped consultantEfforts data:", JSON.stringify(mapped.consultantEfforts, null, 2));
  } catch (err) {
  ...
  await supabase.from('ticket_assignments').delete().eq('ticket_id', ticketId).eq('consultant_id', consultantId);
  await supabase.from('tickets').update({
    status: 'Assigned',
